...
```{r mdimputation, results="hide",echo=FALSE}
...
```{r transform, results="hide", echo=FALSE}
pca_transvars <- vpca_rowpctge(data = dat, 4, ref.P = model.PCA$model$rotation, 
                               k_ho = 20, rm_pctges = c(5,10,seq(20,80,by=20)))

autoencoder_transvars <- vae_rowpctge(data = dat, 4, 
                                               ref.P = model.AE$model$layers[[2]]$weights[[1]], ho.part = pca_transvars$ho,
                                               k_ho = 20, 
...
pca_transvarscox <- vpca_transcols(data = dat, 4, ref.P = model.PCA$model$rotation, 
                               k_ho = 20, rm_pctges = c(5,10,seq(20,80,by=20)))

autoencoder_transvarscox <- vae_transcols(data = dat, 4, 
                                               ref.P = model.AE$model$layers[[2]]$weights[[1]], ho.part = pca_transvarscox$ho,
                                               k_ho = 20, 
                                               rm_pctges = c(5,10,seq(20,80,by=20)))
...
pca_remrows <- vpca_removerows(data = dat, 4, ref.P = model.PCA$model$rotation, 
                               k_ho = 20, rm_pctges = c(2,5,10,seq(20,80,by=20)))

autoencoder_remrows <- vautoencoder_removerows(data = dat, 4, 
                                               ref.P = model.AE$model$layers[[2]]$weights[[1]],
                                               k_ho = 20, ho.part = pca_remrows$ho,
                                               rm_pctges = c(2,5,10,seq(20,80,by=20)))

save(list = c("pca_remrows", "autoencoder_remrows"),file="CoxTrans_models.RData")
```
