...
cairo_ps(file = "AQplotlayers.eps", onefile = FALSE, fallback_resolution = 600,
				  width = 6, height = 4)
lsd_plot
dev.off()
```
...
if(file.exists("../AQref_models.RData") & file.exists(file="../AQref_loadings.RData")){
  ...
  # model.AE <- fit_autoencoder(dat, 4, "relu")
	AQp.hist <- plot(model.AE$trainhist) + theme_minimal() + 
		ggtitle("Training history - Air Quality") + 
		scale_y_continuous(labels = function(x) format(x, scientific = TRUE))
cairo_ps(file = "AEhist_AQplots.eps", onefile = FALSE, fallback_resolution = 600,
				 width = 6, height = 3)
AQp.hist
dev.off()
...
mdi.exp.dataAQ <- rbind(pca_remcells$para_test, autoencoder_remcells$para_test)
mdi.exp.dataAQ$Method <- as.factor(mdi.exp.dataAQ$Method)
mdi.exp.dataAQ$MDpctge <- as.factor(mdi.exp.dataAQ$MDpctge)
mdi.exp.dataAQ$Repetition <- as.factor(mdi.exp.dataAQ$Repetition)
mdi.exp.dataAQ <- mdi.exp.dataAQ[, -which(grepl("^t.",colnames(mdi.exp.dataAQ)))]
mdi.exp.dataAQ <- mdi.exp.dataAQ[, -which(grepl("^lim.",colnames(mdi.exp.dataAQ)))]
mdi.exp.dataAQ <- mdi.exp.dataAQ[, -which(grepl("^log.",colnames(mdi.exp.dataAQ)))]

lsd.results <- lsdAnalysis(mdi.exp.dataAQ, factor1 = "Method", factor2 = "MDpctge", 
...
mdi.plots <- ggpubr::ggarrange(plotlist = plot.results, ncol = 3, nrow = 1,
                  ...
                  common.legend = TRUE)
AQ.exp1.plots <- ggpubr::annotate_figure(mdi.plots, top = ggpubr::text_grob("Case I: Missing Data Imputation - Air Quality", color = "black", face = "bold", size = 12))
#ggsave(AQ.exp1.plots, filename = "CaseI_AQplots.png", width = 8, height = 6, units = "in")
cairo_ps(file = "CaseI_AQplot.eps", onefile = FALSE, fallback_resolution = 600, width = 13, height = 4)
AQ.exp1.plots
dev.off()
```
...
exp3.plots <- ggpubr::ggarrange(plotlist = plot.results.exp3, ncol = 3, nrow = 1,
                  font.label = list(size = 3, color = "black", face = "bold", family = NULL),
                  common.legend = TRUE)
AQ.exp3.plots <- ggpubr::annotate_figure(exp3.plots, top = ggpubr::text_grob("Case II: Column-wise transformation - Air Quality",color = "black", face = "bold", size = 12))
#ggsave(AQ.exp3.plots, filename = "CaseII_AQplots.png", width = 8, height = 6, units = "in")
cairo_ps(file = "CaseII_AQplot.eps", onefile = FALSE, fallback_resolution = 600, width = 13, height = 4)
AQ.exp3.plots
dev.off()
```
...
#ggsave(AQ.exp4.plots, filename = "CaseIII_AQplots.png", width = 8, height = 6, units = "in")
cairo_ps(file = "CaseIII_AQplot.eps", onefile = FALSE, fallback_resolution = 600, width = 13, height = 4)
AQ.exp4.plots
dev.off()
```
