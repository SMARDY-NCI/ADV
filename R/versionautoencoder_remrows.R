versioautoencoder_remrows <- function (data, A, k_ho=1000, rowrm_pctges=c(1,5,10,seq(20,80,by=20))){
  X <- as.matrix(dat)
  library(keras)
  modelA <- keras_model_sequential()
  modelA %>% 
    layer_dense(units=A, activation = "linear", input_shape = ncol(X), 
                use_bias = TRUE, name = "latent") %>%
    layer_dense(units=ncol(X), activation = "linear", input_shape = A, use_bias = TRUE)
  ...
  d.sum[1,] <- paste0(round(colMeans(para_base[, cols.sum,drop=F]), 4),
                      " (",round(apply(para_base[, cols.sum,drop=F], 2, sd), 4), ")")
